...
class JobRegistry:
    ...
    @classmethod
    def discover_plugins(cls, plugins_path: str):
        """Import all modules in the jobs directory to trigger registration."""
        for _, module_name, _ in pkgutil.iter_modules([plugins_path]):
            if module_name == "__init__":
                continue
            try:
                importlib.import_module(f"netraven.worker.jobs.{module_name}")
            except Exception as e:
                dev_log(f"ERROR: Failed to import module '{module_name}': {e}")
                continue

# Discover plugins in the jobs directory
JobRegistry.discover_plugins(JOBS_PATH)