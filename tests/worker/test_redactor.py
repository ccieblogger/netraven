import pytest
from netraven.worker.redactor import redact, REDACTED_LINE_MARKER, REDACTION_KEYWORDS
...
def test_redact(input_str, expected_output):
    """Tests the redact function with various inputs."""
    assert redact(input_str) == expected_output

def test_redaction_keywords_present():
    """Ensures the default keywords are present."""
    assert "password" in REDACTION_KEYWORDS
    assert "secret" in REDACTION_KEYWORDS
